...
Accepted InputSubmittedMsg -> Session applies its atomic post-submit draft -> finishes the partial line -> per-line input hooks, echo, internal dispatch -> grouped effective history
...
- **Presentation belongs to Lua** via `rune.style` (`04_style.lua`). Even the local-echo styling (`"> "` prefix) is a Lua handler on the `"echo"` hook. Go colors only its last-resort degraded-path messages, through `text.Red`/`text.Green` - raw escape codes live in exactly one file per language.
