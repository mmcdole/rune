...
Accepted SubmissionMsg -> Session clears its tracked draft -> finishes current server line -> input/echo processing
Network Read -> Parser.Receive -> transport filters MCCP activation -> optional net.Inbound batch -> Session-owned network.Protocol
Protocol Effects -> Session server-line buffer -> output hook or prompt(line, confirmed)
Accepted Game Line -> Session queues connection-scoped write -> finishes current server line / flushes spans
...
Data-flow: `"output"`, `"prompt"`, `"echo"` support returning `false` to gag or a string to rewrite (rewrites CHAIN to subsequent handlers; the core `"echo"` handler adds the `"> "` styling). `prompt` always receives `(line, confirmed)`: false is a repeatable observation of the current unterminated line at the end of a `network.EventBatch`; true means GA/EOR consumed it as a prompt. Unterminated data followed by GA/EOR in the same batch produces only the confirmed observation. A newline or bare CR instead consumes the line through `output` immediately; an optional LF following CR is swallowed, even when it arrives in a later event or batch. Rune uses no timer or prompt-pattern inference. Every user submission finishes an open current line before history, echo, and input hooks, even when it is local, consumed, disconnected, or ultimately fails to send. A programmatic game line finishes it only after its connection-scoped network write is accepted. If Lua sends while an inbound batch is being processed, the write is queued immediately, but the visual line finish waits for that batch's callbacks to install their final rewrite or gag. Failed sends, GMCP, NAWS, and Telnet negotiation do not finish server text. Every `"input"` handler receives `(text, context)` exactly once per submission, with read-only `context.mode` always `"command"` or `"verbatim"`; verbatim `text` may contain line breaks. Input supports only `false` (consume) - string returns are ignored, and the core input handler at priority 100 always consumes, so custom input handlers must register below 100.
