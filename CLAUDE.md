...
- Every `"input"` handler receives `(text, context)` once unless an earlier handler consumes the submission. The read-only `context.mode` is always `"command"` or `"verbatim"`; verbatim `text` may contain line breaks. A string rewrites the text for later handlers, `nil` or another value passes it through, and `false` consumes it before local echo, history, and dispatch. The final command-mode rewrite must remain ordinary one-line text; structured or terminal-active text is rejected instead of being stored with the wrong mode. Input hooks run before the submission is committed, so they do not see it in history. Every surviving handler runs in priority order; final command/verbatim processing happens only after the hook chain.
