...
- Every `"input"` handler receives `(text, context)` for one physical line in both modes. Read-only `context.mode` is `"command"` or `"verbatim"`. Strings chain, `nil` or other values pass through, and `false` consumes only that line. Newlines in replacements are rejected before later handlers run. Command replacements also reject invalid UTF-8 and terminal controls. Engine.RunSubmission owns validation, splitting, blank-line selection, and the cumulative rewrite budget, then runs hooks, the Session echo callback, and dispatch per line before advancing. History records surviving lines as one entry afterward, so neither input nor echo hooks see the current entry. `RunSubmission` provides one shared watchdog deadline and a private history snapshot for expansion. Public history remains live. Visual wrapping never splits a command.
