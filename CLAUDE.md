...
- Every `"input"` handler receives `(text, context)` once unless an earlier handler consumes the submission. The read-only `context.mode` is always `"command"` or `"verbatim"`; verbatim `text` may contain line breaks. A string rewrites the text for later handlers, `nil` or another value passes it through, and `false` consumes it before local echo, history, and dispatch. Input hooks run before the submission is committed, so they do not see it in history. There is no priority cutoff: internal command/verbatim routing runs only after the full hook chain.
- The named core input hook `history-expansion` runs at priority 100. For interactive command submissions it expands complete `!`, `!!`, and `!prefix` delimiter components against the prior command history. Submissions beginning with `/` bypass it; it also skips slash-command history entries, verbatim entries, and entries containing bang designators. A missing match warns and consumes the whole submission. Programmatic `rune.send("!")` is literal.
