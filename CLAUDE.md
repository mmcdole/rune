...
Accepted InputSubmittedMsg -> Session applies its atomic post-submit draft -> finishes the partial line -> input hooks -> effective history -> echo -> internal dispatch
...
- Every `"input"` handler receives `(text, context)` for each nonblank command line, or once for a whole verbatim draft, unless an earlier handler consumes the submission. The read-only `context.mode` is always `"command"` or `"verbatim"`; verbatim `text` may contain line breaks. A string rewrites the text for later handlers, `nil` or another value passes it through, and `false` consumes it before local echo, history, and dispatch. Command-mode rewrites may contain newlines and tabs. Invalid UTF-8, terminal controls, and oversized results are rejected. All command hook chains finish before the effective block is committed to history and dispatched by physical line. Visual wrapping never splits a command. Input hooks run before the submission is committed, so they do not see it in history. Every surviving handler runs in priority order; final command/verbatim processing happens only after the hook chain.
- The named core input hook `history-expansion` runs at priority 100. For interactive command submissions it expands complete history-designator components separated by `rune.config.get("command_separator")`. The marker is `rune.config.get("history_character")` (`!` by default; empty disables), so the default forms are `!`, `!!`, and `!prefix`. Submissions beginning with `/` bypass it; it also skips slash-command history entries, verbatim entries, and entries containing designators for the current marker. A missing match warns and consumes the whole submission. Programmatic `rune.send` does not run history expansion.
