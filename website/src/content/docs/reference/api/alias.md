...
rune.alias.get(address)                   -- the alias's handle, or nil
```
...
are [addressed](/reference/api/#names): an exact alias is addressed by
its normalized phrase, so `rune.alias.disable("chat off")` works without
naming it, and by its `name` when you give one. A regex alias is a
matcher rather than a phrase, and several can match one line, so it is
...
Standard registry management applies, addressed by name or exact phrase:
`rune.alias.get/enable/disable/remove(address)`, `.list()`, `.count()`,
