...
func commitAndDispatchTestCommand(t *testing.T, engine *Engine, host *MockHost, text string) bool {
	t.Helper()
	effective, proceed := engine.ApplyInputHooks(input.Line{Text: text, Mode: input.ModeCommand})
	if !proceed {
		return false
	}
...
func TestHistoryExpansionPreservesStoredSurroundingWhitespace(t *testing.T) {
	...
	effective, proceed := engine.ApplyInputHooks(input.Line{Text: "!ki", Mode: input.ModeCommand})
...
func TestHistoryExpansionSkipsWhitespaceOnlyHistory(t *testing.T) {
	...
	effective, proceed := engine.ApplyInputHooks(input.Line{Text: "!", Mode: input.ModeCommand})
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, rewrite := range []struct {
				...
				effective, proceed := engine.ApplyInputHooks(input.Line{Text: rewrite.text, Mode: input.ModeCommand})
			...
			if _, proceed := engine.ApplyInputHooks(input.Line{Text: test.marker + "missing", Mode: input.ModeCommand}); proceed {
...
func TestVerbatimBangIsLiteral(t *testing.T) {
	...
	effective, proceed := engine.ApplyInputHooks(input.Line{Text: "!", Mode: input.ModeVerbatim})
