...
-- Shell-style history expansion: `!` (or `!!`) resends the last
-- command, `!prefix` the newest command starting with prefix. As in
-- the shells, history keeps the expansion, not the bang line, so
-- up-arrow recalls the real command. Remove with
-- rune.alias.remove("repeat-last") on MUDs where `!` is a real
-- command; verbatim submissions always bypass aliases.
rune.alias.regex("^!(\\S*)$", function(matches)
    local spec = matches[1]
    if spec == "!" then spec = "" end -- zsh muscle memory: !! = !

    local history = rune.history.get()
    local expanded
    for i = #history, 1, -1 do
        local entry = history[i]
        -- Skip bang lines (including the one just recorded for this
        -- submission): expanding into another `!` could loop.
        if entry:sub(1, 1) ~= "!" then
            if spec == "" or entry:sub(1, #spec) == spec then
                expanded = entry
                break
            end
        end
    ...
    -- Replace the recorded bang line with its expansion. Only when the
    -- bang was the whole submission: a compound line like "!;look" is
    -- one history entry and stays as typed. Programmatic rune.send("!")
    -- records nothing, so there is nothing to replace.
    if history[#history] == "!" .. matches[1] then
        rune._history.pop()
        if expanded then
            rune.history.add(expanded)
        end
    end

    if not expanded then
        rune.echo(rune.style.yellow("[History]") .. " no matching command: !" .. matches[1])
        return
    end
    rune.send(expanded)
end, { name = "repeat-last" })
