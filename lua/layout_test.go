...
func TestLayoutValidationIsStrictAndAtomic(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "name on input", body: `{ type="input", name="input_region" }`, want: `unknown field "name"`},
		...
		{name: "hidden region contains input", body: `{ type="column",children={{type="row",id="input_region",hidden=true,children={{type="pane",name="output"},{type="input",size="1fr"}}},{type="bar",name="status"}} }`, want: `region "input_region" contains input and cannot be hidden`},
