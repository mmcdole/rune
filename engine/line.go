...
func registerLineType(L *lua.LState) {
...
func newLine(L *lua.LState, raw, clean string) *lua.LUserData {
...
func checkLine(L *lua.LState, n int) *Line {
...
var lineMethods = map[string]lua.LGFunction{
	"raw":   lineRaw,
	"line":  lineLine,
...
// Usage: line:raw()
func lineRaw(L *lua.LState) int {
	line := checkLine(L, 1)
	L.Push(lua.LString(line.Raw))
	return 1
}

// lineLine returns the clean line without ANSI codes.
// Usage: line:line() or line:clean()
func lineLine(L *lua.LState) int {
	line := checkLine(L, 1)
	L.Push(lua.LString(line.Clean))
	return 1
}
